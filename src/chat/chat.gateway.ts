...
export class ChatGateway implements OnGatewayInit<Server>, OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleMessagePage(client: Socket, userId: string): Promise<void> {
        ...
        const conversationMessages = await this.conversationsService.getConversationMessages(user._id.toString(), userId)
        client.emit('message', conversationMessages)
    }

    @SubscribeMessage('newMessage')
    async handleNewMessage(client: Socket, data: any): Promise<void> {
        const conversation = await this.conversationsService.findOrCreateConversation(data.sender, data.reciver)
        ...
        await this.conversationsService.addMessageToConversation(conversation._id.toString(), message._id.toString())

        const updatedMessages = await this.conversationsService.getConversationMessages(data.sender, data.receiver)

        this.server.to(data.sender).emit('message', updatedMessages)
        this.server.to(data.receiver).emit('message', updatedMessages)

        const conversationSender = await this.conversationsService.getUserConversations(data.sender)
        const conversationReceiver = await this.conversationsService.getUserConversations(data.receiver)
    ...
    async handleSidebar(client: Socket, currentUserId: string): Promise<void> {
        const conversations = await this.conversationsService.getUserConversations(currentUserId)
    ...
    async handleSeen(client: Socket, msgByUserId: string): Promise<void> {
        ...
        const conversationSender = await this.conversationsService.getUserConversations(userId)
        const conversationReceiver = await this.conversationsService.getUserConversations(msgByUserId)
